...
class GraphBuilderService:
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        """设置图谱本体（公开方法）"""
        entities = {item["name"]: item for item in ontology.get("entity_types", []) if item.get("name")}
        edges = {item["name"]: item for item in ontology.get("edge_types", []) if item.get("name")}
    ...
    def _wait_for_episodes(
        ...
        while pending_episodes:
            ...
            if progress_callback:
                progress_callback(
                    f"Graphiti处理中... {completed_count}/{total_episodes} 完成, {len(pending_episodes)} 待处理 ({elapsed}秒)",
